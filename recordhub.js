...
function startServer() {
  ...
  // ── v2.2.0: Health + Version (external monitoring) ───────────────────────────
  app.get("/health", (_, res) => {
    res.json({
      ok:          true,
      service:     "presenterhub-recordhub",
      version:     "2.2.0",
  ...
  async function gracefulShutdown(signal) {
    ...
    try {
      // Stop OBS recording first if active
      if (STATE.recording) {
        log("Stopping active recording before shutdown...");
        try {
          STATE.pendingSession = STATE.currentSession;
          STATE.currentSession = null;
          await obsStopRecord();
          ...
          log("Could not stop recording cleanly:", e.message);
        }
        // Brief pause for file rename/remux to start
